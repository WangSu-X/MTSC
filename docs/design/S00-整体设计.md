...
MTSC 将这个能力简化为不依赖 UCS Control Plane 的 vLLM KV Connector。实现会参考两个原生 Mooncake Connector，但不实例化、包装或委托给它们：

- Store 数据面直接调用 `MooncakeDistributedStore`，lookup queue、GET/PUT future、completion 和 request ownership 由 MTSC 实现；
- PD 数据面直接调用 `TransferEngine`，bootstrap registry、ZMQ listener、P/D 状态机和 wire protocol 由 MTSC 实现；
- 仅复用 vLLM 内无状态的 key/mask、cache layout 和 topology helper，不复用原生 Connector 的 scheduler/worker state。

vLLM 原生 Mooncake PD Connector 的传输语义是：

```text
D --MTSC PDTransferRequest--> P listener
P --TE batch_transfer_sync_write--> D KV cache
P --MTSC PDTransferResponse--> D
```
...
- 非 MLA 路径使用 TE 传输要求的 HND layout；
...
flowchart LR
    ...
    DC -->|Stage 2 PDTransferRequest| BS
    PC -->|TE WRITE suffix KV| DC
    BS -->|PDTransferResponse| DC
```
